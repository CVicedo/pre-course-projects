...
//Includes all players data
const players = [
    {name: 'Hermenegildo', score: 10},
    {name: 'Mengual', score: 19},
    {name: 'Wenceslao', score: 5},
    {name: 'Angustias', score: 21}
...
//It creates a new entry in the array player with player's name and initial score, wich equals 0.
function pushPlayer(playerName){
    alert('Disfruta con Pasapalabra, ' + playerName + ', y juega con responsabilidad ;-)\n Recuerda que puedes salir de Pasapalabra en cualquier momento escribiendo "end" como respuesta.');
...
function showPlayers(playersRanking){
    ...
    for(let i = 0; i < playersRanking.length; i++){
        playersString = playersString + playersRanking[i].name + ': ' + playersRanking[i].score + ' palabras acertadas.\n'
    }
    return playersString;
}

//Agregates the new score to the players array
function adScore(arrayPlayer, newScore){
    arrayPlayer[arrayPlayer.length -1].score = newScore;
    return arrayPlayer
}
...
function askQuestion(arrayQuestions){
    ...
    do{
        ...
        for(let i = 0; i < arrayQuestions.length; i++){
            ...
            answer = prompt(arrayQuestions[i].question)
            answer = answer.toLowerCase();
                if(answer === ''){
                    do{
                        alert('Debes introducir una respuesta o teclear END para salir del juego')
                        answer = prompt(arrayQuestions[i].question)
                    }while(answer === '');
                }
            }
            if(answer === arrayQuestions[i].answer){
                correctQuestionsCounter++;
                questionsAnsweredCounter++;
                arrayQuestions[i].status = 1;
                questionsPassed.push(arrayQuestions[i].letter);
                alert('¡Respuesta correcta!');
                answer = undefined;
            } else if (answer !== 'pasapalabra' && answer !== arrayQuestions[i].answer && answer !== 'end' && answer !== undefined){
                arrayQuestions[i].status = 1;
                questionsAnsweredCounter++;
                alert('Ohhh, has fallado esta pregunta.')
                answer = undefined;
            } else if (answer === 'pasapalabra'){
                alert('¡PASAPALABRA! Dejamos esta pregunta para la siguiente ronda.')
                answer = undefined;
            } else if (answer === 'end'){
                return 'end'
            }
        }
        if(questionsAnsweredCounter === 27){
            break
        }

    } while(answer !== 'end')
    finalScore = adScore(players, correctQuestionsCounter);
}

//Execution 
alert('Bienvenido/a a Pasapalabra.\nA continuación te mostramos el ranking actual\n\n' + showPlayers(players.sort((a, b) => b.score -a.score)));
pushPlayer(newPlayerName());
if(askQuestion(questions) === 'end'){
    alert(`Has acertado ${correctQuestionsCounter} palabras antes de ser un/a cobarde.`)
}else if(correctQuestionsCounter === 27){
    alert('¡ENHORABUENA! ¡Has acertado todas las preguntas del juego y te llevas el bote!')
    alert(`El ranking final queda de la siguiente forma\n` + showPlayers(finalScore.sort((a, b) => b.score -a.score)));
}else if (correctQuestionsCounter < 27){
    alert(`Enhorabuena, has acertado ${correctQuestionsCounter} palabras.\n`)
    alert(`El ranking final queda de la siguiente forma\n` + showPlayers(finalScore.sort((a, b) => b.score -a.score)));
}
